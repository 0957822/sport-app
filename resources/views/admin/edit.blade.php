...
@section('content')
    <div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div class="bg-white shadow-md rounded-lg">
            ...
            <form action="{{ route('admin.update-exercise', $exercise->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="tags[]" id="tags" multiple
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        @foreach ($tags as $tag)
                            <option value="{{ $tag }}" {{ in_array($tag, $exerciseTags) ? 'selected' : '' }}>
                                {{ $tag }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <div class="mt-1 flex items-center space-x-6">
                        ...
                        @if ($exercise->image_path)
                            <div class="flex-shrink-0">
                                <img src="{{ Storage::url($exercise->image_path) }}" alt="{{ $exercise->title }}"
